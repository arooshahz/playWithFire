#include "Bomb.h"
Bomb::Bomb(int X, int Y,int indexOfPlayer,Game* game, QGraphicsPathItem *parent):X(X),Y(Y),indexOfPlayer(indexOfPlayer),game(game),QGraphicsPixmapItem(parent) {
...
void Bomb::damagePlayer() {
   auto player=game->getPlayers().at(indexOfPlayer);
    ...
    if ((player->y() > this->y()-15 && player->y()+player->boundingRect().height() < this->y()+this->boundingRect().height()+15
         &&  this->x()- 2*player->boundingRect().width()<= player->x()+player->boundingRect().width() && player->x()+player->boundingRect().width() <= this->x()+this->boundingRect().width()+ 3*player->boundingRect().width())
        || ( player->x() > this->x()-15 && player->x()+player->boundingRect().width() < this->x()+this->boundingRect().width()+15
             &&  this->y()- 3*player->boundingRect().height()<= player->y() && player->y()+player->boundingRect().height() <= this->y()+this->boundingRect().height()+ 3*player->boundingRect().height())) {
